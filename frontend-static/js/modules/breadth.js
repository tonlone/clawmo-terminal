...
(function () {
  ...
  /* ── Equal-Weight Breadth Divergence (RSP vs SPY) — mirror of breadth.html.
     Same data (vol-risk.json), hand-rolled SVG via OC_CHART.lineAbs. */
  function ewbdColor(regime) {
    const r = (regime || '').toLowerCase();
    if (/concentrated|hidden/.test(r)) return '#DC2626';
    if (/narrowing/.test(r))           return '#FB923C';
  ...
  function renderEwbdPanel(vr) {
    ...
    const col = ewbdColor(bd.regime);
    const labels = bd.history.map(r => r[0].slice(5));
    const chart = window.OC_CHART ? window.OC_CHART.lineAbs([
    ...
    return `<div class="mod-panel">
      ...
      <div style="font-size:11px;margin-bottom:6px">
        <span style="color:${col};font-weight:700">${bd.regime}</span>
        <span style="color:var(--fg-dim)"> · avg stock (RSP) </span><b class="mono">${bd.rsp_drawdown}%</b>
        <span style="color:var(--fg-dim)"> vs index (SPY) </span><b class="mono">${bd.spy_drawdown}%</b>
        <span style="color:var(--fg-dim)"> from 52-wk high · gap </span><b class="mono" style="color:${col}">${bd.gap > 0 ? '+' : ''}${bd.gap}pp</b>${bd.gap_pct_rank != null ? ` <span style="color:var(--fg-dim)">(${bd.gap_pct_rank}%ile)</span>` : ''}
      </div>
      <div class="chart-wrap">${chart}</div>
      <div class="chart-legend">
        <span><span class="lg-line" style="background:#A78BFA"></span>RSP drawdown (equal-weight, the average stock)</span>
        <span><span class="lg-line" style="background:#60A5FA"></span>SPY drawdown (cap-weight index)</span>
        <span class="chart-note">RSP more underwater than SPY = a few mega-caps masking broad weakness</span>
      </div>
    </div>`;
  }
  ...
  /* Distance from all-time high — terminal mirror of the stocks-app panel.
     Hand-rolled SVG-free bars (plain divs are enough for a horizontal bar field; OC_CHART has
     no bar-from-the-right primitive and the buildEquityCurveChart idiom is for line/scatter).
     Hues match the stocks surface exactly: at-ATH #4ade80 → deep #f87171.
     br.ath may be ABSENT (yfinance outage / cold cache upstream) — degrade to no panel. */
  function renderAthPanel(br) {
    const ath = br && br.ath;
    if (!ath || !Object.keys(ath).length) return '';
    const items = Object.entries(ath)
      .map(([name, v]) => ({ name, ...v }))
      .filter(v => typeof v.pct_from_ath === 'number')
      .sort((a, b) => b.pct_from_ath - a.pct_from_ath);
    if (!items.length) return '';

    const worst = Math.min(...items.map(v => v.pct_from_ath), -1);
    const rows = items.map(v => {
      const col = v.is_new_ath ? '#4ade80' : v.pct_from_ath > -3 ? '#a3e635'
                : v.pct_from_ath > -8 ? '#fbbf24' : v.pct_from_ath > -15 ? '#fb923c' : '#f87171';
      const w = Math.max(0, Math.min(1, v.pct_from_ath / worst)) * 100;
      const badge = v.is_new_ath
        ? '<span style="background:rgba(74,222,128,0.18);color:#4ade80;font-size:9px;font-weight:700;padding:0 3px;border-radius:2px;margin-left:4px">ATH</span>'
        : v.context ? '<span style="background:rgba(139,148,158,0.18);color:var(--fg-dim);font-size:9px;padding:0 3px;border-radius:2px;margin-left:4px">REF</span>' : '';
      const stale = v.stale ? '<span style="color:#fbbf24;font-size:9px;margin-left:3px">⚠</span>' : '';
      const tr = (typeof v.tr_pct_from_ath === 'number')
        ? `<span style="color:var(--fg-dim);font-size:9px;margin-left:5px">TR ${v.tr_pct_from_ath >= 0 ? '+' : ''}${v.tr_pct_from_ath.toFixed(2)}%</span>` : '';
      // Sector NAME is shown beside the ticker, matching breadth.html's "XLV Health Care".
      // Column is 168px (not the web twin's 150px) because the terminal font is denser and
      // "Communication Services" clipped at 120px. Context rows (SPY/QQQ) have no sector name.
      return `<div style="display:grid;grid-template-columns:168px 1fr 118px;align-items:center;gap:6px;padding:2px 0;font-size:11px">
        <span style="color:var(--fg-dim);white-space:nowrap;overflow:hidden;text-overflow:ellipsis">
          <b style="color:var(--fg)">${v.etf}</b>${v.context ? '' : ' ' + v.name}${badge}${stale}</span>
        <span title="Record close ${v.ath_close} on ${v.ath_date} · since ${v.history_start}"
              style="position:relative;height:12px;background:rgba(255,255,255,0.05);border-radius:2px;display:block">
          <span style="position:absolute;right:0;top:0;bottom:0;width:${w.toFixed(1)}%;background:${col};opacity:0.55;border-radius:2px"></span>
        </span>
        <span style="text-align:right;font-family:var(--font-mono,monospace);color:var(--fg)">${v.pct_from_ath >= 0 ? '+' : ''}${v.pct_from_ath.toFixed(2)}%${tr}</span>
      </div>`;
    }).join('');

    const hi = items.filter(v => v.is_new_ath);
    const asOf = items[0].as_of || '';
    return `
      <div class="mod-panel">
        <div class="mod-panel-title">DISTANCE FROM ALL-TIME HIGH</div>
        <div style="padding:6px 10px">
          <div class="small" style="color:var(--fg-dim);font-size:10px;line-height:1.5;margin-bottom:6px">
            Below each sector ETF's record <b>closing</b> price since it listed. 0.0% = new record close.
            Basis: split-adjusted, dividend-<i>un</i>adjusted (headline basis) — high-yield sectors therefore
            read lower than total return (TR) does. Descriptive, not a signal.
          </div>
          ${rows}
          <div class="small" style="margin-top:6px;color:var(--fg-dim);font-size:10px">
            ${hi.length ? `<b style="color:#4ade80">${hi.map(v => v.etf).join(', ')}</b> closed at a new all-time high on ${asOf}.`
                        : `No sector closed at a new all-time high on ${asOf}.`}
          </div>
        </div>
      </div>`;
  }
