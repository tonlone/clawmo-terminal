...
(function (g) {
  ...
  const GLOSSARY = {
    ...
    'WR':        { full: 'Win Rate',                 def: '% of closed trades that were PROFITABLE (return > 0). NOT the take-profit-hit rate — for wide-target patterns (value, pead) only a small % ever reach the full TP target, so the two differ a lot. The TP-hit rate is shown separately as a tooltip.' },
    ...
    'BT WR':       { full: 'Backtest Win Rate',        def: 'Win rate from historical simulation only; % of backtest trades closed PROFITABLE (return > 0). NOT the take-profit-hit rate (the narrower "% that reached the full TP target").' },
    ...
    'LIVE WR':     { full: 'Live Win Rate',            def: 'Win rate from actual closed live trades; % that closed PROFITABLE (return > 0) in real-time' },
    ...
    'AVG WIN':     { full: 'Average Winning Return',   def: 'Mean return % of PROFITABLE trades (return > 0), not only those that reached the full TP target' },
    'AVG LOSS':    { full: 'Average Losing Return',    def: 'Mean return % of LOSING trades (return ≤ 0), not only those that hit the stop-loss (negative number)' },
    ...
    'GROWTH':      { full: 'Growth Rate (Years 1-5)',         def: 'Expected FCF growth held flat for years 1-5, then faded toward the year-10 Fade Rate over years 6-10' },
    'FADE':        { full: 'Fade Rate (Year 10)',             def: 'The growth rate in year 10; growth fades linearly from the year-1 rate down to this over years 6-10, before the terminal value' },
    ...
    /* ── Options Greeks & terms (long/short dual; GEX greeks are DEALER-aggregate) ── */
    'DELTA':       { full: 'Delta (Δ)',              def: 'How much an option price moves per $1 move in the stock; |Δ| ≈ chance of finishing in-the-money. Call +Δ (0→1), put −Δ (0→−1). On the GEX Greeks table these are DEALER-aggregate (Σ × OI × 100), not one contract.' },
    'GAMMA':       { full: 'Gamma (Γ)',              def: 'How fast Delta itself changes per $1 move. Long = convex (helps you); short = losses accelerate. Peaks at-the-money & near expiry. The "G" in GEX.' },
    'THETA':       { full: 'Theta (Θ)',              def: 'Time decay per day. If you are LONG you bleed it (time = enemy); if SHORT you collect it (time = friend). On the table = dealer-aggregate $/day.' },
    'VEGA':        { full: 'Vega (V)',               def: 'P&L per +1% implied vol. Long gains when IV rises (a "storm"); short gains when IV falls ("IV crush" = payday).' },
    'CALL':        { full: 'Call option',            def: 'Right to BUY 100 shares at the strike. Buyer wants price UP; seller is obligated to deliver shares if assigned.' },
    'PUT':         { full: 'Put option',             def: 'Right to SELL 100 shares at the strike. Buyer wants price DOWN. Selling a put = obligation to BUY 100 shares @ strike if assigned (cash-secured put).' },
    'CALL WALL':   { full: 'Call Wall',              def: 'Strike with the largest call gamma above spot — tends to act as resistance / a price ceiling (pin).' },
    'PUT WALL':    { full: 'Put Wall',               def: 'Strike with the largest put gamma below spot — tends to act as support / a price floor.' },
    'GAMMA FLIP':  { full: 'Gamma Flip',             def: 'The price where net dealer gamma flips sign. Above it = stabilizing (rangey); below it = amplifying (trendy). Reclaiming it = "market calms down".' },
    'GAMMA REGIME':{ full: 'Gamma Regime',           def: 'Positive = dealer hedging dampens moves (dips bought, pins near big strikes). Negative = hedging amplifies moves (trends run, gaps bigger).' },
    'ITM':         { full: 'In-the-Money',           def: 'Option has intrinsic value (call: spot > strike; put: spot < strike).' },
    'OTM':         { full: 'Out-of-the-Money',       def: 'Option has no intrinsic value yet — only time value (call: spot < strike; put: spot > strike).' },
    'ATM':         { full: 'At-the-Money',           def: 'Strike ≈ spot. Gamma and time-decay are largest here.' },
    'PREMIUM':     { full: 'Premium',                def: 'The option price = intrinsic value + time value. Buyer\'s max loss; seller\'s max gain.' },
    'TIME VALUE':  { full: 'Time Value',             def: 'The part of premium above intrinsic value — what Theta melts to zero by expiry.' },
    'INTRINSIC VALUE': { full: 'Intrinsic Value',    def: 'The in-the-money amount: call = max(0, spot − strike), put = max(0, strike − spot).' },
    ...
    'Δ':           { full: 'Delta / Change',          def: 'A change or difference in a value (e.g. Δ RATE = 1-week change in analyst rating). For the options Greek "Delta", see the DELTA entry / the Greeks table.' },
