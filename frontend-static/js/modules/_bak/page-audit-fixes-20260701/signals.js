    ...
    try {
      ...
      function sortHeatmap(patterns, col, dir) {
        return patterns.slice().sort((a, b) => {
          ...
          if (col === 'status') {
            const s = x => x.mothballed ? 'M' : x.quarantined ? 'Q' : x.passes_gate === false ? 'B' : 'A';
      ...
      function buildHeatmapContent() {
        ...
        const rows = sorted.map(p => {
          ...
          const status = p.mothballed ? '<span class="sig-blocked" title="Mothballed — retired from live trading">MOTHBALLED</span>'
                       : p.quarantined ? `<span class="sig-quarantine">Q-${p.quarantine_tier || ''}</span>`
          ...
          return `<tr>
            ...
            <td class="mono" title="Win Rate = % of historical trades closed PROFITABLE (return > 0)${p.tp_hit_rate != null ? ' · TP-hit ' + p.tp_hit_rate.toFixed(0) + '% reached the full take-profit target' : ''}">${overallWr != null ? overallWr.toFixed(0) + '%' : '—'}</td>
            ...
            <div class="chart-legend">
              ...
              <span class="chart-note" style="display:block">
                ...
                <span style="display:block;margin-top:0.4rem;padding-top:0.35rem;border-top:1px dashed var(--border);color:var(--fg-dim);font-size:9.5px;line-height:1.45">
                  <b>Now backtested (Task #10 cross-sectional harness, 2026-06-19 step=1):</b> <code>mom_top_decile</code> (PF 1.43, OOS 1.81 — grade A), <code>ibs_oversold_pullback</code> (PF 1.76, OOS 1.65 stable — grade A), <code>low_vol_top_decile</code> (PF 0.62 — grade D, retired: no long-only edge). <b>Also now backtested (2026-06-20):</b> <code>pead_drift</code> from FMP earnings history (point-in-time SUE, no lookahead) — PF 1.68, OOS-stable 1.56→1.80, p_block 0.0001 over 1,484 trades → <b>published, grade A</b> (lifted the live-only B cap); <code>value_top_decile</code> backtested PF 1.48 but <b>regime-specific</b> (recovery-only) → held, not published. <b>Still not shown</b>: <code>analyst_upgrade_drift</code>, <code>target_upside_top_decile</code> (C1), and <code>earnings_run_up</code> (validated separately by the historical-earnings backtest, PF 1.262).
                  These rank tickers against each other on a given day (or need external/forward data), not per-ticker time series, so the per-ticker backtest harness can't score them. Tracked live on the Grades tab. The cross-sectional harness (Task #10) is <b>done</b> for the 3 price-only patterns above; <code>value_top_decile</code> stays live-only/held (regime-specific) pending the 07-07 grandfather-vs-lock decision. A separate <b>long-short</b> backtest (F8, 2026-06-19) found the momentum/low-vol decile spread has no market-neutral alpha — momentum's edge is long-only, so these remain long-only events (research only, nothing published).
                </span>
      ...
      const sigStrip = sigMeta ? `
        <div class="sig-rigor-strip" style="display:flex;gap:14px;flex-wrap:wrap;font-size:11px;padding:6px 10px;background:var(--bg-elev);border-bottom:1px solid var(--border);font-family:var(--font-mono)">
          ...
          <span title="Patterns whose MCPT (i.i.d.) p remains < 0.05 after Benjamini-Hochberg FDR correction">FDR&lt;0.05 PASS <b style="color:${sigMeta.n_pass_fdr_05 > 0 ? '#4ade80' : 'var(--fg-dim)'}">${sigMeta.n_pass_fdr_05}/${sigMeta.n_tests}</b></span>
          ${sigMeta.n_pass_block_fdr_05 != null ? `<span title="Same FDR pass count but on the i.i.d.-robust moving-block bootstrap p — the honest number for clustered signals">BLK-FDR&lt;0.05 PASS <b style="color:${sigMeta.n_pass_block_fdr_05 > 0 ? '#4ade80' : 'var(--fg-dim)'}">${sigMeta.n_pass_block_fdr_05}/${sigMeta.n_tests}</b></span>` : ''}
          ...
          ${sigMeta.n_pass_dsr_95 != null ? `<span title="Patterns whose Deflated Sharpe ≥ 0.95 — true Sharpe beats the expected max of ${sigMeta.dsr_n_trials ?? '?'} trial patterns (selection-bias control)">DSR ≥ 0.95 <b style="color:${sigMeta.n_pass_dsr_95 > 0 ? '#4ade80' : 'var(--fg-dim)'}">${sigMeta.n_pass_dsr_95}</b></span>` : ''}
          ${sigMeta.reality_check_p != null ? `<span title="White Reality Check (approx): family-wise p that the best pattern (${escSig(sigMeta.reality_check_best || '')}) survives the search">RC p <b style="color:${sigMeta.reality_check_p < 0.05 ? '#4ade80' : 'var(--fg-dim)'}">${sigMeta.reality_check_p.toFixed(3)}</b></span>` : ''}
          ...
          <div class="tbl-wrap">
            <table class="tbl-dense">
              <thead><tr>
                ...
                <th class="num" title="Backtest win rate = % of trades closed profitable (return > 0); NOT the take-profit-hit rate">WR</th>
                ...
                <th class="num" title="MCPT p-value: Monte Carlo permutation test (assumes i.i.d. trades). Green &lt;0.05, yellow &lt;0.10">p(MCPT)</th>
                <th class="num" title="Moving-block bootstrap p: i.i.d.-robust companion to MCPT — resamples contiguous date-sorted blocks, handling overlapping/clustered signals. Usually ≥ p(MCPT). Green &lt;0.05.">p(BLK)</th>
                ...
                <th class="num" title="Deflated Sharpe Ratio: % confidence true Sharpe beats the expected max of all trial patterns by luck — deflates for configs tried across Patches A-G. ≥95% survives selection bias. n≥30.">DSR</th>
              ...
              <tbody>${backtestRows || '<tr><td colspan="19" class="empty">no backtest data</td></tr>'}</tbody>
            </table>
          </div>
          <div class="chart-legend">
            <span class="chart-note" style="display:block">
              <span style="display:grid;grid-template-columns:1fr 1fr;gap:0.1rem 1.2rem;margin-bottom:0.35rem">
                ...
                <span><b>WR</b> — win rate % (closed trades profitable; not the TP-hit rate)</span>
                ...
                <span><b>p(MCPT)</b> — Monte Carlo permutation p-value (assumes i.i.d. trades). Green &lt;0.05, yellow &lt;0.10</span>
                <span><b>p(BLK)</b> — moving-block bootstrap p: i.i.d.-robust fix for overlapping/clustered signals. Usually ≥ p(MCPT) — the honest p. Green &lt;0.05</span>
                ...
                <span><b>DSR</b> — Deflated Sharpe: probability true Sharpe beats the expected max of all trial patterns by luck. Green ≥95% survives the search across Patches A-G. n≥30</span>
      ...
      function buildEquityCurveChart(curve, closed) {
        ...
        // Real-only: exclude paper trades (locked-pattern quarantine fires, no capital) to match
        // the web equity default — else the terminal shows a paper-dragged number for the same
        // account. Recompute alpha cumulative since the precomputed value was all-inclusive.
        curve = (curve || []).filter(p => !p.is_paper);
        closed = (closed || []).filter(t => !t.is_paper);
        let _cumA = 0;
        curve = curve.map(p => Object.assign({}, p, { cumulative: Math.round((_cumA += (p.return_pct || 0)) * 100) / 100 }));
      ...
      const closedRows = closedTrades.slice(closedSliceStart, closedSliceStart + TRADES_PER_PAGE).map(t => {
        ...
        return `<tr>
          <td class="tk clickable" data-tk="${escSig(t.ticker)}">${escSig(t.ticker)}${t.is_paper ? ' <span title="Paper trade — fired while its pattern was quarantined/mothballed, so no capital was allocated. Excluded from the equity curve; shown for tracking only." style="font-size:9px;font-weight:700;color:var(--muted,#8b949e);border:1px solid var(--border,#30363d);border-radius:3px;padding:0 3px;vertical-align:middle">PAPER</span>' : ''}</td>
          <td class="pat">${prettyPat(t.signal_type)}</td>
          <td class="${dirC}">${t.direction === 'long' ? '▲' : '▼'}</td>
        ...
        <div class="mod-panel">
          ...
          <div class="sig-how-meth">
            ...
            <div class="sig-how-meth-row">
              <b>2026-06-20 — staged stop tightened: stage-1 breakeven → +0.5R lock (LIVE)</b> — at the +1R peak the trailing stop now locks <b>+0.5R of profit</b> instead of moving to breakeven (stages 2/3 unchanged). Resolved by a full-universe A/B (n=430,622 setups): the +0.5R lock beat breakeven <b>monotonically</b> (PF 1.172 → 1.199), held out-of-sample across 3 regime splits (incl. the 2022 bear), left the loss tail unchanged (worst-trade loss &amp; CVaR5% identical — the initial stop, not the BE lever, owns the tail), and improved <b>all 15 patterns</b>. Live trade management (<code>update-trade-status.py</code>) and the backtest replica both use +0.5R from today; the full pattern set's backtest was <b>re-run on this basis the same day</b> (manual run of the weekly chain) — every pattern's backtest PF nudged up <b>+0.00 to +0.12</b> (PF-grade bins unchanged). The displayed <b>adaptive</b> grades refresh at the next weekday compute-signals run, where the +0.5R basis nudges a couple of <b>regime-derived</b> grades (e.g. <code>range_breakout</code> B→A as PF crosses 1.20; <code>adam_eve_dbottom</code> A→B as its BULL-slice edge compresses) — small and expected, separate from normal live-performance drift.
            </div>
            <div class="sig-how-meth-row">
              <b>2026-06-20 — PEAD drift backtested + PUBLISHED → grade A (LIVE)</b> — the post-earnings-drift pattern was data-blocked (no historical earnings-surprise data). Built a strict no-lookahead backtest from FMP earnings history (point-in-time SUE on actual EPS announced before each trade) → <b>PF 1.68, OOS stable 1.56→1.80, p_block 0.0001</b> over 1,484 trades (graded on the +0.5R live-exit basis). pead was <b>live-only, capped at B</b> (a thin live record can't earn A without a backtest); publishing the backtest lifts that cap → blended PF 1.75 = <b>grade A, live now</b> (a <b>backtest-graded</b> A — the live record is still thin, n=6, so the grade rests on the 1,484-trade backtest, not live outperformance). Pulled forward from Mon 06-22 (Tony OK'd).
            </div>
            <div class="sig-how-meth-row">
              <b>2026-06-20 — cointegration pairs layer probed → no edge (research, nothing shipped)</b> — tested a market-neutral statistical-arb pairs strategy with strict no-lookahead (form cointegrated within-sector pairs on a 2021–24 train window, trade out-of-sample 2024–26 on frozen params, net of cost+borrow). <b>No robust edge</b> — selected pairs scored PF 0.75 OOS, and a <b>random-pair benchmark scored identically</b> (PF 0.75), so cointegration selection added zero value — the pairs strategy itself has no edge here. Not built. Closes the Tier C institutional-upgrade research list (F8 long-short, F6/F7 risk overlays, pairs); focus stays on PEAD / pre-earnings run-up.
            </div>
            <div class="sig-how-meth-row">
              <b>2026-06-19 — F7 factor concentration (shadow logging)</b> — measures the factor clustering the GICS <b>sector</b> cap can't see (observational, flip pending calibration): risk-unit-weighted <b>beta buckets</b> + <b>momentum buckets</b>, plus a statistical <b>common-factor</b> read from the book's return-correlation matrix — PC1 variance share and the <b>effective number of bets</b> ENB=(Σλ)²/Σλ². Striking baseline: the book is <b>202 names but only ~13.7 effective bets</b> (PC1 = 23% of variance) — far less diversified than the count suggests. Beta/momentum buckets well-spread (none over the 50% cap) → no trim today; ENB is the actionable number.
            </div>
            <div class="sig-how-meth-row">
              <b>2026-06-19 — F6 risk overlay (shadow logging)</b> — two new portfolio-risk measures now log each run (observational, change nothing yet — ~2wk calibration before any live flip, same shadow→flip arc as Piece C): a <b>20d SPY realized-vol target</b> that would de-gross the risk-unit ceiling when realized vol runs above the 15% target (a direct, leading vol signal vs the existing stop-cluster de-gross), and a <b>portfolio beta cap</b> = the risk-unit-weighted signed net beta of the open book vs SPY, with a highest-beta trim sim. Baseline: SPY 20d vol 16.2% → ceiling 350→324; book beta 0.90 (100% long, BULL) — under the 1.0 cap, no trim.
            </div>
            <div class="sig-how-meth-row">
              <b>2026-06-19 — Phase 2 Piece C: heat cap → risk units</b> — the portfolio heat ceiling flipped from a raw <b>position count (350)</b> to a grade-weighted <b>risk-unit ceiling (MAX_RISK_UNITS = 350)</b>: each open/new position consumes A=1.25 / B=1.0 / C=0.75 / D=0.5 units, grade from backtest PF (same source as the priority dedup below). Tony picked N=350 (conservative) after ~2 weeks of shadow logging. <b>No-op at current load</b> (book peaks ~200 positions / ~222 RU vs 350) — its value is a grade-aware ceiling for the day a strong BULL run fills the book (grade-A costs more, D less). Falls back to the count gate when no grade context exists. Live next weekday compute-signals run.
            </div>
            <div class="sig-how-meth-row">
              <b>2026-06-19 — F8 long-short + grade-aware priority</b> — <b>F8 (research, nothing published):</b> tested the momentum &amp; low-vol decile factors as a market-neutral <b>long(top-decile) − short(bottom-decile)</b> book (net of cost, survivorship-free). <b>No market-neutral alpha</b> — momentum spread PF 0.95 / IR −0.05, and its edge is <b>entirely in the long leg</b> (long beats SPY +0.58%/mo; the bottom-decile short mean-reverts up &amp; injects −0.61 beta), so <code>mom_top_decile</code> stays a <b>long-only</b> grade-A event. Low-vol long-short is a −2.04× beta trap (PF 0.52) → confirms retirement. No live long-short product; the measured spread betas feed the upcoming beta-cap work. <b>Grade-aware concentration priority:</b> the per-ticker / capacity dedup (Concentration Cap stage) now picks the higher-<b>grade</b> setup first, confidence only breaking ties within a grade — a validated grade-A pattern no longer loses its slot to a higher-confidence but lower-grade chart pattern. Backward-compatible (confidence-only fallback when no backtest grade exists). Live next weekday compute-signals run.
            </div>
            <div class="sig-how-meth-row">
            ...
            <div class="sig-how-meth-row">
              ...
              <ul style="margin:4px 0 0 18px; padding:0;">
                ...
                <li><b>momentum_rest_entry</b> (Luk #7 / Qullamaggie rest) — parabolic run then tight green rest bar. Small-sample pattern (fires via the Patch G count override; see card below for current grade).</li>
              </ul>
            </div>
            <div class="sig-how-meth-row">
