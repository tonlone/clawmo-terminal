...
(function () {
  ...
  async function renderSentiment(body, ctx) {
    ...
    try {
      const [d, preds, div, pc, vr, si, fgm] = await Promise.all([
        ...
        fetchJSON(`${BASE}/vol-risk.json`).catch(() => null),
        fetchJSON(`${BASE}/short-interest.json`).catch(() => null),
        fetchJSON(`${BASE}/fear-greed-market.json`).catch(() => null),
      ]);
      window._senPutCall = pc;
      ...
      body.innerHTML = `
        ...
        <div data-mod-panel="sen">
          ...
          ${fgm && (fgm.history || []).length ? `<div class="mod-panel" data-fgm-panel>${buildFGMarketPanel(fgm, '3y')}</div>` : ''}
          ...
          ${vr ? `<div class="mod-panel">${buildVolPanel(vr)}</div>` : ''}

          ${pc ? `<div class="mod-panel" data-pc-panel>${buildPutCallPanel(pc, 'total')}</div>` : ''}

          ${si && (si.history || []).length ? `<div class="mod-panel">${buildShortIntPanel(si)}</div>` : ''}
  ...
  /* ── Volatility Regime (mirror of stocks sentiment.html vol panel) ───────
     Same data & thresholds; hand-rolled SVG via OC_CHART.lineAbs (no Chart.js). */
  function volColor(regime) {
    const r = (regime || '').toLowerCase();
    if (/backwardation|extreme|stress|concentrated/.test(r)) return '#DC2626';
    if (/elevated|narrowing|tipping/.test(r))                return '#FB923C';
    if (/deep contango|calm|broad/.test(r))                  return '#5BB77A';
    return '#60A5FA';
  }
  function volStat(lbl, val, pctile, regime) {
    const col = regime ? volColor(regime) : 'var(--fg-dim)';
    return `<div class="vol-stat">
      <div class="vol-lbl">${lbl}</div>
      <div class="vol-val">${val}</div>
      <div class="vol-sub">${pctile != null ? pctile + '%ile' : ''}${regime ? ` · <span style="color:${col}">${regime}</span>` : ''}</div>
    </div>`;
  }
  function buildVolPanel(vr) {
    const v = vr.vol || {}, bd = vr.breadth_divergence || {};
    const ts = v.term_structure || {}, tsp = v.tech_spread || {};
    const stats = [];
    if (v.vix)  stats.push(volStat('VIX', v.vix.value, v.vix.pct_rank, null));
    if (ts.value != null) stats.push(volStat('TERM (VIX÷VIX3M)', ts.value, ts.pct_rank, ts.regime));
    if (tsp.value != null) stats.push(volStat('TECH (VXN−VIX)', (tsp.value > 0 ? '+' : '') + tsp.value, tsp.pct_rank, null));
    if (v.skew) stats.push(volStat('SKEW', v.skew.value, v.skew.pct_rank, v.skew.regime));
    if (v.vvix) stats.push(volStat('VVIX', v.vvix.value, v.vvix.pct_rank, v.vvix.regime));
    // term-structure ratio line with a dashed 1.0 backwardation reference
    const tsHist = ts.history || [];
    const tsLabels = tsHist.map(r => r[0].slice(5));
    const tsChart = (window.OC_CHART && tsHist.length) ? window.OC_CHART.lineAbs([
      { name: 'VIX÷VIX3M', values: tsHist.map(r => r[1]), color: '#60A5FA' },
      { name: 'Backwardation (1.0)', values: tsHist.map(() => 1.0), color: '#DC2626', dashed: true },
    ], { gridY: 3, xLabels: tsLabels, yFmt: x => x.toFixed(2) }) : '';
    const skHist = (v.skew && v.skew.history) || [];
    const skChart = (window.OC_CHART && skHist.length) ? window.OC_CHART.lineAbs([
      { name: 'SKEW', values: skHist.map(r => r[1]), color: '#FB923C' },
      { name: 'Elevated (145)', values: skHist.map(() => 145), color: '#DC2626', dashed: true },
    ], { gridY: 3, xLabels: skHist.map(r => r[0].slice(5)), yFmt: x => x.toFixed(0) }) : '';
    const bdCol = volColor(bd.regime);
    const bdLine = (bd.gap != null) ? `<div class="vol-bd">Equal-weight breadth divergence:
      RSP <b class="mono">${bd.rsp_drawdown}%</b> vs SPY <b class="mono">${bd.spy_drawdown}%</b> from 52-wk high ·
      gap <b class="mono" style="color:${bdCol}">${bd.gap > 0 ? '+' : ''}${bd.gap}pp</b> ·
      <span style="color:${bdCol}">${bd.regime}</span> <span style="color:var(--fg-dim)">(see BRD)</span></div>` : '';
    return `<style>
      [data-mod-panel="sen"] .vol-strip { display:grid; grid-template-columns:repeat(5,1fr); gap:6px; margin:4px 0 10px; }
      @media (max-width:640px){ [data-mod-panel="sen"] .vol-strip { grid-template-columns:repeat(2,1fr); } }
      [data-mod-panel="sen"] .vol-stat { background:var(--pane-bg); border:1px solid var(--border); border-radius:3px; padding:6px 8px; }
      [data-mod-panel="sen"] .vol-lbl { font-size:9px; color:var(--fg-dim); text-transform:uppercase; letter-spacing:0.5px; }
      [data-mod-panel="sen"] .vol-val { font-size:16px; font-weight:700; font-family:var(--font-mono); margin-top:1px; }
      [data-mod-panel="sen"] .vol-sub { font-size:9px; color:var(--fg-dim); margin-top:1px; }
      [data-mod-panel="sen"] .vol-bd { font-size:10px; color:var(--fg-dim); margin-top:8px; padding-top:6px; border-top:1px solid var(--border); }
      [data-mod-panel="sen"] .vol-charts { display:grid; grid-template-columns:1fr 1fr; gap:10px; }
      @media (max-width:640px){ [data-mod-panel="sen"] .vol-charts { grid-template-columns:1fr; } }
      </style>
      <div class="mod-panel-title">VOLATILITY REGIME · descriptive context, not a signal</div>
      <div class="vol-strip">${stats.join('')}</div>
      <div class="vol-charts">
        <div><div style="font-size:10px;color:var(--fg-dim);margin-bottom:2px">VIX term structure · &lt;1 contango (calm) · &gt;1 backwardation</div><div class="chart-wrap">${tsChart}</div></div>
        <div><div style="font-size:10px;color:var(--fg-dim);margin-bottom:2px">CBOE SKEW · crash-hedging demand</div><div class="chart-wrap">${skChart}</div></div>
      </div>
      ${bdLine}`;
  }

  /* ── CBOE Put/Call Ratio vs SPY (mirror of stocks sentiment.html) ────────
     Same data, hues and threshold semantics; hand-rolled SVG (no Chart.js). */
  const PC_META = {
    total:  { label: 'Total',  color: '#60A5FA' },
    equity: { label: 'Equity', color: '#A78BFA' },
    index:  { label: 'Index',  color: '#FB923C' },
    etp:    { label: 'ETP',    color: '#5BB77A' },
  };
  function pcBand(pct) {
    if (pct == null) return ['—', 'var(--fg-dim)'];
    if (pct >= 95) return ['FEAR EXTREME', '#5BB77A'];
    if (pct >= 80) return ['ELEVATED', '#86efac'];
    if (pct <= 5)  return ['GREED EXTREME', '#DC2626'];
    if (pct <= 20) return ['COMPLACENT', '#fca5a5'];
    return ['NEUTRAL', 'var(--fg-dim)'];
  }
  function buildPutCallSVG(pc, series) {
    const meta = PC_META[series], hist = pc.history || [];
    if (hist.length < 10) return '<div class="pane-empty">No data</div>';
    const W = 780, H = 210, padL = 34, padR = 40, padT = 10, padB = 20;
    const ratio = hist.map(h => h[series]);
    const spy = hist.map(h => h.spy);
    const ma = ratio.map((_, i) => { if (i < 9) return null; let s = 0; for (let j = i - 9; j <= i; j++) s += ratio[j]; return s / 10; });
    const ctx = (pc.context || {})[series] || {};
    const thr = (pc.thresholds || {})[series] || [];
    // ratio Y-range (left axis) — include thresholds so lines are visible
    const rvals = ratio.filter(v => typeof v === 'number');
    let rMin = Math.min(...rvals, ...thr), rMax = Math.max(...rvals, ...thr);
    const rPad = (rMax - rMin) * 0.05 || 0.1; rMin -= rPad; rMax += rPad;
    // spy Y-range (right axis)
    const svals = spy.filter(v => typeof v === 'number');
    let sMin = Math.min(...svals), sMax = Math.max(...svals);
    const n = hist.length;
    const sx = i => padL + (i / (n - 1)) * (W - padL - padR);
    const ry = v => padT + (1 - (v - rMin) / (rMax - rMin)) * (H - padT - padB);
    const syy = v => padT + (1 - (v - sMin) / (sMax - sMin)) * (H - padT - padB);
    const pathOf = (arr, yfn) => {
      let d = '', started = false;
      arr.forEach((v, i) => { if (typeof v !== 'number') { started = false; return; }
        d += (started ? 'L' : 'M') + sx(i).toFixed(1) + ' ' + yfn(v).toFixed(1) + ' '; started = true; });
      return d;
    };
    // threshold lines + percentile band
    const thrLines = thr.map(t => `<line x1="${padL}" y1="${ry(t).toFixed(1)}" x2="${W - padR}" y2="${ry(t).toFixed(1)}" stroke="rgba(148,163,184,0.35)" stroke-width="1" stroke-dasharray="4 3"></line><text x="${W - padR + 2}" y="${(ry(t) + 3).toFixed(1)}" fill="var(--fg-dim)" font-size="9">${t}</text>`).join('');
    let band = '';
    if (ctx.p5 != null && ctx.p95 != null) {
      const y95 = ry(ctx.p95), y5 = ry(ctx.p5);
      band = `<rect x="${padL}" y="${y95.toFixed(1)}" width="${W - padL - padR}" height="${Math.max(0, y5 - y95).toFixed(1)}" fill="rgba(96,165,250,0.05)"></rect>`;
    }
    // extreme markers
    const ev = (pc.extremes || {})[series]; let marks = '';
    if (ev) {
      const dateIdx = {}; hist.forEach((h, i) => dateIdx[h.date] = i);
      const tri = (d, up) => { const i = dateIdx[d]; if (i == null) return '';
        const x = sx(i), y = ry(ratio[i]), c = up ? '#5BB77A' : '#DC2626';
        return up ? `<path d="M${x} ${y - 7} l4 7 l-8 0 z" fill="${c}"></path>`
                  : `<path d="M${x} ${y + 7} l4 -7 l-8 0 z" fill="${c}"></path>`; };
      marks = (ev.fear.episodes || []).map(e => tri(e.date, true)).join('') +
              (ev.greed.episodes || []).map(e => tri(e.date, false)).join('');
    }
    const yTicks = [rMin, (rMin + rMax) / 2, rMax].map(v => `<text x="${padL - 3}" y="${(ry(v) + 3).toFixed(1)}" fill="${meta.color}" font-size="9" text-anchor="end">${v.toFixed(2)}</text>`).join('');
    const sTicks = [sMin, sMax].map(v => `<text x="${W - padR + 2}" y="${(syy(v) + 3).toFixed(1)}" fill="var(--fg-dim)" font-size="9">${v.toFixed(0)}</text>`).join('');
    return `<svg class="oc-chart" viewBox="0 0 ${W} ${H}" preserveAspectRatio="none">
      ${band}${thrLines}
      <path d="${pathOf(spy, syy)}" style="fill:none;stroke:rgba(139,148,158,0.8);stroke-width:1"></path>
      <path d="${pathOf(ratio, ry)}" style="fill:none;stroke:${meta.color};stroke-width:0.9;opacity:0.55"></path>
      <path d="${pathOf(ma, ry)}" style="fill:none;stroke:#E5B94C;stroke-width:1.6"></path>
      ${marks}${yTicks}${sTicks}
    </svg>`;
  }
  function pcAccTable(ev) {
    const pct = x => x == null ? '—' : (x >= 0 ? '+' : '') + (x * 100).toFixed(1) + '%';
    const row = (side, lbl) => {
      const s = ev[side].summary || {}, nEp = (ev[side].episodes || []).length;
      const cells = [5, 21, 63].map(h => { const d = s['fwd' + h] || {};
        if (!d.n) return '<td>—</td>';
        const star = d.unstable ? '<span style="color:#FB923C" title="n<12, exploratory">*</span>' : '';
        return `<td class="mono">${pct(d.median)} · ${(d.hit_rate * 100).toFixed(0)}% · n${d.n}${star}</td>`; }).join('');
      return `<tr><td>${lbl} <span style="color:var(--fg-dim)">(${nEp})</span></td>${cells}</tr>`;
    };
    return `<table class="tbl-dense mod-table" style="width:100%;font-size:11px"><thead><tr><th>${ev.series === 'equity' ? 'Equity' : 'Total'} extreme</th><th>+5d</th><th>+21d</th><th>+63d</th></tr></thead><tbody>${row('fear', '<span style="color:#5BB77A">▲</span> Fear → buy')}${row('greed', '<span style="color:#DC2626">▼</span> Greed → sell')}</tbody></table>`;
  }
  function buildPutCallPanel(pc, series) {
    const meta = PC_META[series];
    const ctx = (pc.context || {})[series] || {};
    const [bl, bc] = pcBand(ctx.now_pctile);
    const btns = Object.keys(PC_META).map(s =>
      `<button class="sen-pred-btn${s === series ? ' active' : ''}" data-pc-series="${s}">${PC_META[s].label}</button>`).join('');
    return `
      <div class="mod-panel-title">CBOE PUT/CALL RATIO vs SPY · ${pc.latest_date || ''} · contrarian sentiment</div>
      <div style="margin:4px 0 8px">${btns}</div>
      <div style="display:flex;gap:18px;flex-wrap:wrap;margin-bottom:6px;font-size:11px">
        <span>latest <b class="mono" style="color:${meta.color}">${ctx.now ?? '—'}</b></span>
        <span>10d MA <b class="mono">${ctx.ma10 ?? '—'}</b></span>
        <span>pctile <b class="mono">${ctx.now_pctile == null ? '—' : ctx.now_pctile + '%'}</b></span>
        <span class="chip" style="background:${bc};color:#0d1117;font-weight:700">${bl}</span>
      </div>
      <div class="chart-wrap" data-pc-svg>${buildPutCallSVG(pc, series)}</div>
      <div style="margin-top:8px">${pcAccTable(pc.extremes.total)}</div>
      <div style="margin-top:6px">${pcAccTable(pc.extremes.equity)}</div>
      <div style="font-size:9px;color:var(--fg-dim);margin-top:6px;line-height:1.5"><span style="color:#5BB77A">green ▲ = fear</span> (raw ≥ fear line, Total 1.20 → contrarian BUY, bottoms); <span style="color:#DC2626">red ▼ = greed</span> (raw ≤ greed line, Total 0.70 → contrarian SELL, tops). Deduped into episodes. Forward SPY return · hit-rate · n. <span style="color:#FB923C">*</span>=n&lt;12. Descriptive not predictive — ~6.7y ≈ one cycle, 3 horizons untested for multiplicity. Equity-only = traditional gauge; Total matches MacroMicro.</div>`;
  }
  /* ── Aggregate Short Interest (mirror of stocks sentiment.html SII panel) ──
     Same data (short-interest.json), same framing, same caveats. DESCRIPTIVE ONLY —
     no signals coupling, no predictive claim. Hand-rolled SVG (no Chart.js here). */
  function buildShortIntSVG(si) {
    const hist = si.history || [];
    if (hist.length < 2) return '<div class="mod-err">no history</div>';
    const W = 700, H = 200, padL = 34, padR = 34, padT = 8, padB = 18;
    const z = hist.map(h => h.sii_z);
    const spy = hist.map(h => (typeof h.spy === 'number' ? h.spy : null));
    const zvals = z.filter(v => typeof v === 'number');
    let zMin = Math.min(...zvals), zMax = Math.max(...zvals);
    const zPad = (zMax - zMin) * 0.05 || 0.1; zMin -= zPad; zMax += zPad;
    const svals = spy.filter(v => typeof v === 'number');
    const hasSpy = svals.length > 1;
    const sMin = hasSpy ? Math.min(...svals) : 0, sMax = hasSpy ? Math.max(...svals) : 1;
    const n = hist.length;
    const sx = i => padL + (i / (n - 1)) * (W - padL - padR);
    const zy = v => padT + (1 - (v - zMin) / (zMax - zMin)) * (H - padT - padB);
    const syy = v => padT + (1 - (v - sMin) / (sMax - sMin)) * (H - padT - padB);
    const pathOf = (arr, yfn) => {
      let d = '', started = false;
      arr.forEach((v, i) => { if (typeof v !== 'number') { started = false; return; }
    ...
    // zero line = the detrended mean
    const zeroY = (zMin <= 0 && zMax >= 0) ? zy(0) : null;
    const zeroLine = zeroY == null ? '' :
      `<line x1="${padL}" y1="${zeroY.toFixed(1)}" x2="${W - padR}" y2="${zeroY.toFixed(1)}" stroke="rgba(148,163,184,0.35)" stroke-width="1" stroke-dasharray="4 3"></line>`;
    const yTicks = [zMin, (zMin + zMax) / 2, zMax].map(v =>
      `<text x="${padL - 3}" y="${(zy(v) + 3).toFixed(1)}" fill="#f472b6" font-size="9" text-anchor="end">${v.toFixed(1)}</text>`).join('');
    const sTicks = !hasSpy ? '' : [sMin, sMax].map(v =>
      `<text x="${W - padR + 2}" y="${(syy(v) + 3).toFixed(1)}" fill="var(--fg-dim)" font-size="9">${v.toFixed(0)}</text>`).join('');
    const xTicks = [0, Math.floor(n / 2), n - 1].map(i => {
      const anchor = i === 0 ? 'start' : (i === n - 1 ? 'end' : 'middle');
      return `<text x="${sx(i).toFixed(1)}" y="${H - 5}" fill="#8b949e" font-size="9" text-anchor="${anchor}" font-family="monospace">${(hist[i].date || '').slice(0, 7)}</text>`;
    }).join('');
    const spyPath = hasSpy
      ? `<path d="${pathOf(spy, syy)}" style="fill:none;stroke:rgba(139,148,158,0.8);stroke-width:1"></path>` : '';
    return `<svg class="oc-chart" viewBox="0 0 ${W} ${H}" preserveAspectRatio="none">
      ${zeroLine}${spyPath}
      <path d="${pathOf(z, zy)}" style="fill:none;stroke:#f472b6;stroke-width:1.6"></path>
      ${yTicks}${sTicks}${xTicks}
    </svg>`;
  }
  function buildShortIntPanel(si) {
    const c = si.context || {}, mp = si.mirror_probe || {}, pn = si.panel || {};
    const band = c.now_pctile >= 90 ? ['ELEVATED', '#FB923C']
               : c.now_pctile <= 10 ? ['DEPRESSED', '#60A5FA']
               : ['MIDDLING', '#8b949e'];
    return `
      <div class="mod-panel-title">AGGREGATE SHORT INTEREST (FINRA) · settlement ${c.settlement_date || '—'} · positioning</div>
      <div style="display:flex;gap:18px;flex-wrap:wrap;margin:6px 0;font-size:11px">
        <span>short interest <b class="mono" style="color:#f472b6">${c.now_pct_shares_out ?? '—'}%</b> of shares out</span>
        <span>SII <b class="mono">z=${c.now_z ?? '—'}</b></span>
        <span>pctile <b class="mono">${c.now_pctile == null ? '—' : c.now_pctile + '%'}</b></span>
        <span class="chip" style="background:${band[1]};color:#0d1117;font-weight:700">${band[0]}</span>
        <span style="color:var(--fg-dim)">${c.n_names ?? '—'} names · <b>${c.lag_days ?? '?'}d old</b></span>
      </div>
      <div class="chart-wrap">${buildShortIntSVG(si)}</div>
      <div style="font-size:9px;color:var(--fg-dim);margin-top:6px;line-height:1.5">
        <span style="color:#f472b6">pink</span> = SII (equal-weighted short interest as % of shares outstanding across a frozen
        ${pn.n ?? '—'}-name large-cap panel, detrended + standardized, per Rapach-Ringgenberg-Zhou 2016);
        <span style="color:#8b949e">grey</span> = SPY. Coverage from ${si.coverage_start || '—'}.
        <b>Descriptive not predictive.</b> Reading is <b>9–23 days stale by construction</b> (bi-monthly settlement +
        ~8–9 business day publication lag) — the date shown is the SETTLEMENT date, not the fetch date.
        Days-to-cover is deliberately NOT used: its volume denominator inverts it at stress (near-lowest reading at the
        COVID bottom). Not a VIX mirror — SII vs VIX+SKEW+21d-ret R²=${mp.r2_combined ?? '—'} (n=${mp.n ?? '—'}).
        Panel frozen ${pn.frozen_at || '—'} ⇒ names since acquired/delisted are absent (survivorship; bias direction on an
        "N-year high" ambiguous). RRZ read high short interest bearishly — <b>we make no such call</b>; untested here and
        wired to no signal.
      </div>`;
  }

  /* ── CNN F&G vs S&P 500 & VIX overlay (mirror of stocks sentiment.html) ───
     Same fear-greed-market.json, same episodes/stats (backend is the single
     source — no recompute here). Hand-rolled dual-axis SVG (no Chart.js). */
  const FGM_FG = '#60A5FA', FGM_VIX = '#E5B94C', FGM_SPX = 'rgba(220,38,38,0.85)';
  function fgmSliceT(hist, range) {
    if (range === 'all' || !hist.length) return hist;
    const yrs = range === '1y' ? 1 : 3;
    const last = new Date(hist[hist.length - 1].date);
    const cut = new Date(last); cut.setFullYear(cut.getFullYear() - yrs);
    const cutStr = cut.toISOString().slice(0, 10);
    return hist.filter(h => h.date >= cutStr);
  }
  function fgRatingT(v) {
    if (v == null) return '—';
    if (v <= 25) return 'Extreme Fear'; if (v <= 45) return 'Fear';
    if (v <= 55) return 'Neutral'; if (v <= 75) return 'Greed'; return 'Extreme Greed';
  }
  function buildFGMarketSVG(fgm, range) {
    const hist = fgmSliceT(fgm.history || [], range);
    if (hist.length < 10) return '<div class="pane-empty">No data</div>';
    const W = 780, H = 230, padL = 30, padR = 44, padT = 10, padB = 20;
    const fg = hist.map(h => h.fg), vix = hist.map(h => h.vix), spx = hist.map(h => h.spx);
    const n = hist.length;
    const svals = spx.filter(v => typeof v === 'number');
    let sMin = Math.min(...svals), sMax = Math.max(...svals);
    const sPad = (sMax - sMin) * 0.03 || 1; sMin -= sPad; sMax += sPad;
    const sx = i => padL + (i / (n - 1)) * (W - padL - padR);
    const fy = v => padT + (1 - v / 100) * (H - padT - padB);          // fixed 0..100 (F&G + VIX)
    const syy = v => padT + (1 - (v - sMin) / (sMax - sMin)) * (H - padT - padB);
    const pathOf = (arr, yfn) => {
      let d = '', started = false;
      arr.forEach((v, i) => { if (typeof v !== 'number') { started = false; return; }
    ...
    const zl = [25, 75].map(p => `<line x1="${padL}" y1="${fy(p).toFixed(1)}" x2="${W - padR}" y2="${fy(p).toFixed(1)}" stroke="rgba(148,163,184,0.3)" stroke-width="1" stroke-dasharray="4 3"></line>`).join('');
    const dateIdx = {}; hist.forEach((h, i) => dateIdx[h.date] = i);
    const mk = (eps, color, diamond) => (eps || []).map(e => {
      const i = dateIdx[e.onset_date]; if (i == null) return '';
      const x = sx(i), y = fy(fg[i] != null ? fg[i] : e.onset_fg);
      return diamond ? `<path d="M${x} ${(y - 6).toFixed(1)} l5 6 l-5 6 l-5 -6 z" fill="${color}"></path>`
                     : `<path d="M${x} ${(y - 7).toFixed(1)} l4 7 l-8 0 z" fill="${color}"></path>`;
    }).join('');
    const ev = fgm.extremes || {};
    const marks = mk((ev.fear || {}).episodes, '#5BB77A', false) + mk((ev.fear_vix || {}).episodes, '#FB923C', true);
    const yTicks = [0, 25, 50, 75, 100].map(v => `<text x="${padL - 3}" y="${(fy(v) + 3).toFixed(1)}" fill="${FGM_FG}" font-size="9" text-anchor="end">${v}</text>`).join('');
    const sTicks = [sMin, sMax].map(v => `<text x="${W - padR + 2}" y="${(syy(v) + 3).toFixed(1)}" fill="var(--fg-dim)" font-size="9">${v.toFixed(0)}</text>`).join('');
    const xTicks = [0, Math.floor(n / 2), n - 1].map(i => {
      const anchor = i === 0 ? 'start' : (i === n - 1 ? 'end' : 'middle');
      return `<text x="${sx(i).toFixed(1)}" y="${H - 5}" fill="#8b949e" font-size="9" text-anchor="${anchor}" font-family="monospace">${(hist[i].date || '').slice(0, 7)}</text>`;
    ...
    return `<svg class="oc-chart" viewBox="0 0 ${W} ${H}" preserveAspectRatio="none">
      ${zl}
      <path d="${pathOf(spx, syy)}" style="fill:none;stroke:${FGM_SPX};stroke-width:1"></path>
      <path d="${pathOf(fg, fy)}" style="fill:none;stroke:${FGM_FG};stroke-width:1.3"></path>
      <path d="${pathOf(vix, fy)}" style="fill:none;stroke:${FGM_VIX};stroke-width:1"></path>
      ${marks}${yTicks}${sTicks}${xTicks}
    </svg>`;
  }
  function fgmAccTable(ev, label) {
    const pct = x => x == null ? '—' : (x >= 0 ? '+' : '') + (x * 100).toFixed(1) + '%';
    const s = ev.summary || {}, b = ev.base_rate || {};
    const cells = [5, 21, 63].map(h => {
      const d = s['fwd' + h] || {}, bd = b['fwd' + h] || {};
      if (!d.n) return '<td>—</td>';
      const ci = d.hit_ci95_block ? ` [${(d.hit_ci95_block[0] * 100).toFixed(0)}–${(d.hit_ci95_block[1] * 100).toFixed(0)}]` : '';
      const base = bd.base_up_rate != null ? ` vs ${(bd.base_up_rate * 100).toFixed(0)}%` : '';
      const star = d.unstable ? '<span style="color:#FB923C" title="n<12, exploratory">*</span>' : '';
      return `<td class="mono">${pct(d.median)} · ${(d.hit_rate * 100).toFixed(0)}%${ci}${base} · n${d.n}${star}</td>`;
    }).join('');
    return `<tr><td>${label} <span style="color:var(--fg-dim)">(${ev.n_episodes})</span></td>${cells}</tr>`;
  }
  function buildFGMarketPanel(fgm, range) {
    const cur = (fgm.history || [])[(fgm.history || []).length - 1] || {};
    const rangeBtns = ['1y', '3y', 'all'].map(r =>
      `<button class="sen-pred-btn${r === range ? ' active' : ''}" data-fgm-range="${r}">${r.toUpperCase()}</button>`).join('');
    const prov = fgm.provisional_date === cur.date ? ' <span class="chip" style="background:var(--fg-dim);color:#0d1117">today provisional</span>' : '';
    return `
      <div class="mod-panel-title">CNN FEAR &amp; GREED vs S&amp;P 500 &amp; VIX · ${fgm.latest_date || ''} · attention context, not a signal</div>
      <div style="margin:4px 0 8px">${rangeBtns}</div>
      <div style="display:flex;gap:18px;flex-wrap:wrap;margin-bottom:6px;font-size:11px;align-items:center">
        <span>F&amp;G <b class="mono" style="color:${FGM_FG}">${cur.fg ?? '—'}</b></span>
        <span>VIX <b class="mono" style="color:${FGM_VIX}">${cur.vix ?? '—'}</b></span>
        <span>S&amp;P <b class="mono" style="color:#DC2626">${cur.spx ?? '—'}</b></span>
        <span class="chip" style="background:${fgColor(cur.fg)};color:#0d1117;font-weight:700">${fgRatingT(cur.fg)}</span>${prov}
      </div>
      <div class="chart-wrap" data-fgm-svg>${buildFGMarketSVG(fgm, range)}</div>
      <div style="margin-top:8px"><table class="tbl-dense mod-table" style="width:100%;font-size:11px">
        <thead><tr><th>Extreme-fear onset</th><th>+5d</th><th>+21d</th><th>+63d</th></tr></thead>
        <tbody>
          ${fgmAccTable(fgm.extremes.fear, '<span style="color:#5BB77A">▲</span> F&amp;G&lt;25')}
          ${fgmAccTable(fgm.extremes.fear_vix, '<span style="color:#FB923C">◆</span> F&amp;G&lt;25 &amp; VIX top-10%')}
        </tbody></table></div>
      <div style="font-size:9px;color:var(--fg-dim);margin-top:6px;line-height:1.5"><span style="color:#5BB77A">green ▲</span> = extreme-fear onset (F&amp;G&lt;25); <span style="color:#FB923C">orange ◆</span> = + VIX top-10% (post-2021). Forward S&amp;P return · hit-rate [block-bootstrap 95% CI] vs base rate · n. Markers sit at ONSET, not the hindsight low. ${(fgm.note || '').replace(/</g, '&lt;')}</div>`;
  }
  function wireFGMarketPanel(body, fgm) {
    const panel = body.querySelector('[data-fgm-panel]');
    if (!panel) return;
    panel.addEventListener('click', e => {
      const b = e.target.closest('[data-fgm-range]'); if (!b) return;
      panel.querySelectorAll('[data-fgm-range]').forEach(x => x.classList.toggle('active', x === b));
      const wrap = panel.querySelector('[data-fgm-svg]');
      if (wrap) wrap.innerHTML = buildFGMarketSVG(fgm, b.dataset.fgmRange);
    });
  }

  function wirePutCallPanel(body, pc) {
    const panel = body.querySelector('[data-pc-panel]');
    if (!panel) return;
    panel.addEventListener('click', e => {
      const b = e.target.closest('[data-pc-series]'); if (!b) return;
      panel.innerHTML = buildPutCallPanel(pc, b.dataset.pcSeries);
    });
  }
