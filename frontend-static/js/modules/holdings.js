...
(function () {
  ...
  function congressRow(t) {
    return `<tr>
      ...
      <td class="mono">${escapeHtml(fmtAmountRange(t.amount))}</td>
    </tr>`;
  }
  ...
  /* FMP hands the disclosed amount through verbatim, and a few filings arrive
     with the upper bound missing — "$15,001 -" (11 of 4291 as of 2026-08, all
     but two from one filer). Rendered raw that looks like a clipped cell, so
     close the range explicitly instead. */
  function fmtAmountRange(a) {
